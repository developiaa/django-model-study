	...
	def create(self, request, *args, **kwargs):
		try:
			...
			if serializer.is_valid():
				...
				comment = Comment.objects.filter(id=comment_id, post_id=post_id, depth=1).first()
				if comment:
					sequence_max = Comment.objects.filter(group=comment.group, post_id=post_id).aggregate(
						Max('sequence'))
